...
<div class="panel-heading">
    <ul class="nav nav-pills nav-danger">
        <li <?php if (StringUtils::isBlank($type)): ?>class="active" <?php endif; ?>>
            <?php echo Html::a("所有",ArrayHelper::merge(['banner/index'], ArrayHelper::merge($params, ['BannerSearch[type]' => ''])))  ?>
        </li>
        <?php foreach (Banner::$typeArr as  $key => $value): ?>
            <li <?php if ($type == $key&&!StringUtils::isBlank($type)): ?>class="active" <?php endif; ?> >
                <?php echo Html::a($value,ArrayHelper::merge(['banner/index'], ArrayHelper::merge($params, ['BannerSearch[type]' => $key])))  ?>
            </li>
        <?php endforeach; ?>
    </ul>
</div>