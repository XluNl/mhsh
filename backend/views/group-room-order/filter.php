...
<div class="panel-heading">
    <ul class="nav nav-tabs">
        <li <?php if (StringUtils::isBlank($order_status)): ?>class="active" <?php endif; ?>>
            <?php echo Html::a("所有",ArrayHelper::merge(['group-room-order/index'], ArrayHelper::merge($params, ['GroupRoomOrderSearch[order_status]' => ''])))  ?>
        </li>
        <?php foreach (Order::$order_status_list as $key => $value): ?>
            <li <?php if ($order_status == $key&&!StringUtils::isBlank($order_status)): ?>class="active" <?php endif; ?> >
                <?php echo Html::a($value,ArrayHelper::merge(['group-room-order/index'], ArrayHelper::merge($params, ['GroupRoomOrderSearch[order_status]' => $key])))  ?>
            </li>
        <?php endforeach; ?>
    </ul>
</div>