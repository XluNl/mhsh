...
class CaptchaService
{
    ...
    public static function sendCaptcha($data,$sort){
        ...
        /*if ($sort==Captcha::SORT_SMS_CUSTOMER){
            $userInfo = UserInfoService::getActiveUserInfoByPhone($data);
            ExceptionAssert::assertNull($userInfo,StatusCode::createExpWithParams(StatusCode::SEND_CAPTCHA_ERROR,'该手机号已注册'));
        }
        else if ($sort==Captcha::SORT_SMS_DELIVERY){

        }*/
        $captcha = self::getCaptcha($data,Captcha::SORT_SMS_CUSTOMER);
        self::sendSMS($data,$captcha,Captcha::SORT_SMS_CUSTOMER);
        return $captcha;
    }
